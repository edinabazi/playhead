    ...
    return (
      <DropdownContext.Provider value={{ registerItem, activeIndex, checkedIndex }}>
        <div
          ...
          role="menu"
          className={cn(
            `relative flex flex-col gap-0.5 w-72 max-w-full ${shape.container} bg-card shadow-[0_4px_12px_rgba(0,0,0,0.02)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.5)] border border-border/60 p-1`,
            className,
          )}
          {...props}
        >
          ...
          <AnimatePresence>
            {activeRect && (
              <motion.div
                ...
                className={`absolute ${shape.bg} bg-[var(--menu-hover-background)] pointer-events-none`}
                initial={{
                  opacity: 0,
